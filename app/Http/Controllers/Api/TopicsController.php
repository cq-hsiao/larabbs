...
//            ->allowedIncludes('user', 'category')
        ...
        $topics->transform(function ($customer) use ($include) {
//            unset($customer->user);
//            $aa = $customer->setHidden(['user','category']);
//            dd($aa);
            $diff = array_diff(array_keys($customer->getRelations()),$include);
            foreach ($diff as $item){
                $customer->unsetRelation($item);
            }
            return $customer;
        });
